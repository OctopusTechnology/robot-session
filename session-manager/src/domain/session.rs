...
impl Session {
    ...
    pub async fn create_livekit_room(&self, config: &LiveKitConfig) -> Result<()> {
        ...
        tracing::debug!("  Room options: empty_timeout={}s, max_participants={}",
            options.empty_timeout, options.max_participants);
        ...
        match room_client.create_room(&self.room_name, options).await {
            ...
            Err(e) => {
                tracing::error!("✗ Failed to create LiveKit room '{}': {}", self.room_name, e);
    ...
    pub async fn delete_livekit_room(&self, config: &LiveKitConfig) -> Result<()> {
        ...
        match room_client.delete_room(&self.room_name).await {
            ...
            Err(e) => {
                tracing::error!("✗ Failed to delete LiveKit room '{}': {}", self.room_name, e);
                Err(SessionManagerError::LiveKit(e))
            }
        }
    }
    ...
    pub async fn connect_to_livekit(&mut self, livekit_config: LiveKitConfig, event_bus: Arc<EventBus>) -> Result<()> {
        tracing::debug!("Connecting session {} to LiveKit room", self.id);
        tracing::debug!("  Room name: {}", self.room_name);
        tracing::debug!("  Expected microservices: {}", self.registered_microservices.len());
        ...
        let (room, event_rx) = Room::connect(&ws_url, &room_token, RoomOptions::default()).await
            .map_err(|e| {
                tracing::error!("✗ Failed to connect to LiveKit room: {}", e);
                SessionManagerError::Internal(anyhow::anyhow!("Failed to connect to LiveKit room: {}", e))
            })?;

        tracing::info!("✓ Successfully connected to LiveKit room: {}", self.room_name);

        // Start monitoring participants
        let session_id = self.id.clone();
        let expected_services: HashSet<String> = self.registered_microservices
            .iter()
            .map(|s| s.service_id.clone())
            .collect();
        
        tracing::debug!("Starting lifecycle monitoring for {} expected services", expected_services.len());
        
        let event_bus_clone = event_bus.clone();
        let event_handle = tokio::spawn(async move {
            Self::monitor_session_lifecycle(session_id, event_rx, expected_services, event_bus_clone).await;
        });
        ...
        tracing::info!("✓ Session {} connected to LiveKit and monitoring started", self.id);
        Ok(())
    }
    ...
    async fn monitor_session_lifecycle(
        ...
        let mut service_last_seen: std::collections::HashMap<String, std::time::Instant> = std::collections::HashMap::new();
        ...
        let mut retry_timer = tokio::time::interval(std::time::Duration::from_secs(SERVICE_RETRY_INTERVAL_SECS));
        
        tracing::info!("Starting session lifecycle monitoring for session {}", session_id);
        ...
        tracing::info!("Session lifecycle monitoring ended for session {}", session_id);
    }
    ...
    fn generate_room_token(&self, config: &LiveKitConfig) -> Result<String> {
        ...
        let token = AccessToken::with_api_key(&config.api_key, &config.api_secret)
            ...
            .map_err(|e| {
                ...
                SessionManagerError::Internal(anyhow::anyhow!("Failed to generate room token: {}", e))
            })?;
    ...
    pub fn generate_client_token(&self, config: &LiveKitConfig) -> Result<String> {
        ...
        let token = AccessToken::with_api_key(&config.api_key, &config.api_secret)
            ...
            .map_err(|e| {
                ...
                SessionManagerError::Internal(anyhow::anyhow!("Failed to generate client token: {}", e))
            })?;

        tracing::info!("✓ Client token generated successfully for session {}", self.id);
        Ok(token)
    }
    ...
    pub async fn notify_microservices_to_join(&self, livekit_config: &LiveKitConfig, livekit_url: &str) -> Result<()> {
        if self.registered_microservices.is_empty() {
            tracing::debug!("No microservices to notify for session {}", self.id);
            return Ok(());
        }

        tracing::info!("Notifying {} microservices to join session {}",
            self.registered_microservices.len(), self.id);
        ...
        for service in &self.registered_microservices {
            ...
            let access_token = self.generate_microservice_token(&service.service_id, livekit_config)?;
            ...
            tokio::spawn(async move {
                tracing::debug!("Sending join notification to service {} at {}", service_id, service_endpoint);
                match Self::notify_service_join(service_endpoint, join_request).await {
                    Ok(()) => {
                        tracing::info!("✓ Successfully sent join notification to service {}", service_id);
                    }
                    Err(e) => {
        ...
        tracing::info!("✓ Sent join notifications to all {} microservices for session {}",
            self.registered_microservices.len(), self.id);
    ...
    fn generate_microservice_token(&self, service_id: &str, config: &LiveKitConfig) -> Result<String> {
        ...
        tracing::debug!("  Grants: room_join=true, can_publish=true, can_subscribe=true, can_publish_data=true");
        ...
        let token = AccessToken::with_api_key(&config.api_key, &config.api_secret)
            ...
            .to_jwt()
            .map_err(|e| {
                tracing::error!("✗ Failed to generate microservice token for {}: {}", service_id, e);
                SessionManagerError::Internal(anyhow::anyhow!("Failed to generate microservice token: {}", e))
            })?;

        tracing::debug!("✓ Microservice token generated successfully for {}", service_id);
        Ok(token)
    }

    /// Notify a single service to join the room
    async fn notify_service_join(endpoint: String, request: crate::domain::JoinRoomRequest) -> Result<()> {
        ...
        if status.is_success() {
            ...
            tracing::error!("✗ Service at {} returned error {}: {}", endpoint, status, error_text);
            Err(SessionManagerError::Internal(anyhow::anyhow!(
                "Service returned error {}: {}", status, error_text
            )))
        }
    }
    ...
    pub async fn disconnect_from_livekit(&mut self) -> Result<()> {
        ...
        tracing::info!("✓ Session {} disconnected from LiveKit and terminated", self.id);
        Ok(())
    }
}