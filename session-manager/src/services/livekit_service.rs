...
impl LiveKitService {
    ...
    pub async fn create_room(&self, room_name: &str) -> Result<()> {
        ...
        self.room_client
            ...
            .await
            .map_err(SessionManagerError::LiveKit)?;
    ...
    pub async fn join_room_as_manager(&self, room_name: &str, session_id: &str) -> Result<()> {
        ...
        tracing::info!(
            "Session manager connected to room: {} as {}",
            room_name,
            manager_identity
        );
        ...
        let event_handle = tokio::spawn(async move {
            while let Some(event) = event_rx.recv().await {
                Self::handle_room_event(event, &event_bus, &room_name_clone, &session_id_clone)
                    .await;
            }
        ...
        self.room_connections
            .write()
            .await
            .insert(room_name.to_string(), room_connection);
        ...
        match event {
            RoomEvent::ParticipantConnected(participant) => {
                ...
                if let Err(e) = event_bus
                    .publish_participant_joined(session_id, &identity)
                    .await
                {
            ...
            RoomEvent::ParticipantDisconnected(participant) => {
                ...
                // 发布参与者离开事件
                if let Err(e) = event_bus
                    .publish_participant_left(session_id, &identity)
                    .await
                {
            ...
            RoomEvent::Connected { .. } => {
                tracing::info!(
                    "Session manager successfully connected to room: {}",
                    room_name
                );
            }
            RoomEvent::Disconnected { reason } => {
                tracing::warn!(
                    "Session manager disconnected from room {}: {:?}",
                    room_name,
                    reason
                );
            }
    ...
    pub async fn get_joined_microservices(
        &self,
        room_name: &str,
